  ...
  >
    <a-form :form="form">
      ...
      <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol">
        <span slot="label">断言器名字
          <a-tooltip :title="currentPredicateObject.objectTips" v-if="currentPredicateObject && currentPredicateObject.objectTips">
            <a-icon type="question-circle-o" />
          </a-tooltip>
        </span>
        <a-select placeholder="请选择断言" style="width: 97%" v-decorator="['name', {rules: [{ required: true, message: '请选择断言' }]}]" @change="predicateTypeChange($event)">
          <a-select-option v-for="(item) in this.predicateObjects" :key="item.objectName" :value="item.objectName">{{ item.objectDesc }}</a-select-option>
        </a-select>
      </a-form-item>
...
import Vue from 'vue'
import { objectList, predicateValidCheck } from '@/api/gateway/route'

export default {
  name: 'PredicateSaveModal',
  props: {},
  components: {},
  data () {
    return {
      ...
      predicateObjects: null,
      showTypeOne: false,
      showTypeTwo: false,
      addArgAble: false,
      mdl: {},
      argsList: [],
      currentPredicateObject: null,
      labelName: '',
  ...
  methods: {
    ...
    init () {
      objectList({ objectType: 'predicate' }).then(res => {
        if (res.code === '200' && res.data && res.data.length > 0 && this.checkObjectRes(res.data)) {
          this.predicateObjects = res.data
          this.predicateObjects.forEach(object => {
    ...
    resetParam () {
      ...
      this.argsList = []
      this.currentPredicateObject = {}
      this.labelName = ''
    },
    refreshAddArgAble () {
      const objectArgsDefine = this.currentPredicateObject.objectArgsDefine[0]
    ...
    addPredicate (index) {
      this.editPredicate({ newFlag: true, index: index + 1 })
    },
    editPredicate (predicate) {
      ...
      this.resetParam()
      this.mdl = Object.assign(predicate)
      predicate.newFlag = predicate.newFlag ? predicate.newFlag : false
      if (!predicate.newFlag) {
        const predicateObject = this.predicateObjects.filter(function (item) {
          return item.objectName === predicate.name
        })[0]
        this.currentPredicateObject = predicateObject
        const objectArgsDefine = predicateObject.objectArgsDefine
        if (predicate.name && predicateObject.objectArgType === 'List') {
          ...
          if (predicate.args && objectArgsDefine[0].argValueControlType !== 'fixed') {
            const argsData = JSON.parse(predicate.args)
        ...
        if (predicate.name && predicateObject.objectArgType === 'Map') {
        ...
        if (predicateObject.objectArgType === 'Map') {
          objectArgsDefine.forEach(argDefine => {
            let find = false
    ...
    predicateTypeChange (value) {
      ...
      this.currentPredicateObject = predicateObject
      const objectArgsDefine = predicateObject.objectArgsDefine
      if (predicateObject.objectArgType === 'List') {
        this.showTypeOne = true
        const scope = {
          argTips: objectArgsDefine[0].argTips,
          argValueControlType: objectArgsDefine[0].argValueControlType,
    ...
    addArg () {
      const currentObject = this.currentPredicateObject
    ...
    checkInput () {
      if (this.argsList.length === 0) {
        this.$message.error('至少添加一条参数！')
        return false
      }
      let flag = true
      const argInputs = Array.from(document.getElementsByClassName('argInput'))
      if (argInputs.length > 0) {
        const argValueControlType = this.currentPredicateObject.objectArgsDefine[0].argValueControlType
        argInputs.forEach(item => {
    ...
    handleSubmit (e) {
      ...
      this.form.validateFields((err, values) => {
        if (!err && this.checkInput()) {
          ...
          predicateValidCheck({ predicateStr: JSON.stringify(data) }).then(res => {
            if (res.code === '200') {
