<template>
  ...
  >
    ...
    <Predicate-save-modal @cancel="receivePredicateCancel" ref="predicateSaveModal" @ok="handleOk"/>
  </a-modal>
</template>

<script>
import { STable } from '@/components'
import PredicateSaveModal from './PredicateSaveModal.vue'

export default {
  name: 'PredicatesModal',
  components: {
    STable,
    PredicateSaveModal
  },
  ...
  methods: {
    ...
    handleAdd (index) {
      this.$refs.predicateSaveModal.addPredicate(index)
    },
    handleEdit (predicate) {
      if (predicate.args === '无') {
        delete predicate.args
      }
      this.$refs.predicateSaveModal.editPredicate(predicate)
    },
    handleOk () {
      this.$refs.table.refresh(true)
    },
    receivePredicateCancel (data) {
    ...
    handleSubmit () {
      ...
      if (argsEmptyFlag === true) {
        this.$message.error('断言参数不能为空！')
        return
      }
