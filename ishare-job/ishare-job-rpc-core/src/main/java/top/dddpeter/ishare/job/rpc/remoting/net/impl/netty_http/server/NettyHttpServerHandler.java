package top.dddpeter.ishare.job.rpc.remoting.net.impl.netty_http.server;
...
public class NettyHttpServerHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
        ...
        try {
            if ("/services".equals(uri)) {	// services mapping
                ...
                if (requestBytes.length == 0) {
                    throw new RpcException("ishare-rpc request data empty.");
                }
                ...
                if (Beat.BEAT_ID.equalsIgnoreCase(rpcRequest.getRequestId())){
                    logger.debug(">>>>>>>>>>> ishare-rpc provider netty_http server read beat-ping.");
                    return;
                }
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) {
        logger.error(">>>>>>>>>>> ishare-rpc provider netty_http server caught exception", cause);
        ctx.close();
    }
    ...
    public void userEventTriggered(ChannelHandlerContext ctx, Object evt) throws Exception {
        if (evt instanceof IdleStateEvent){
            ...
            logger.debug(">>>>>>>>>>> ishare-rpc provider netty_http server close an idle channel.");
        } else {
