package top.dddpeter.ishare.job.admin.core.thread;
...
public class JobFailMonitorHelper {
        ...
        monitorThread = new Thread(() -> {
            ...
            while (!toStop) {
                try {
                    ...
                    if (!toStop) {
                        logger.error(">>>>>>>>>>>ishare-job, job fail monitor thread error:{}", e);
                    }
                }
            ...
            logger.info(">>>>>>>>>>>ishare-job, job fail monitor thread stop");
    ...
    private static final String MAIIL_BODY_TEMPLATE = "<h5>" + I18nUtil.getString("jobconf_monitor_detail") + "：</span>" +
    ...
    private boolean failAlarm(JobInfo info, JobLog jobLog) {
        ...
        if (info != null && info.getAlarmEmail() != null && info.getAlarmEmail().trim().length() > 0) {
            ...
            String content = MessageFormat.format(MAIIL_BODY_TEMPLATE,
            ...
            // make mail
            try {
                RemoteMessageService messageService = JobAdminConfig.getAdminConfig().getMessageService();
                CommonSendEmail sendEmail = new CommonSendEmail();
                sendEmail.setTemplateNo("COMMON_JOB_EXECUTE_ERROR_EMAIL_SEND");
                sendEmail.setEmailList(new ArrayList<>(emailSet));
                Map<String, String> params = new HashMap<>();
                params.put("title", title);
                params.put("content", content);
                sendEmail.setParam(params);
                ResultDTO resultDTO = messageService.sendEmail(sendEmail);
                if (!resultDTO.isSuccess()) {
                    throw new Exception(resultDTO.getMessage());
                }
            } catch (Exception e) {
                logger.error(">>>>>>>>>>>ishare-job, job fail alarm email send error, JobLogId:{}", jobLog.getId(), e);
