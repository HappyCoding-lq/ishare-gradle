...
public class JobScheduleHelper {
    ...
    public void start(){
        ...
        scheduleThread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                logger.info(">>>>>>>>> initishare-job admin scheduler success.");
                ...
                while (!scheduleThreadToStop) {
                    ...
                    try {
                        ...
                        if (scheduleList!=null && scheduleList.size()>0) {
                            ...
                            for (JobInfo jobInfo: scheduleList) {
                                ...
                                if (nowTime > jobInfo.getTriggerNextTime() + PRE_READ_MS) {
                                    ...
                                    logger.warn(">>>>>>>>>>>ishare-job, schedule misfire, jobId = " + jobInfo.getId());
                                    ...
                                    logger.debug(">>>>>>>>>>>ishare-job, schedule push trigger : jobId = " + jobInfo.getId() );
                        ...
                        if (!scheduleThreadToStop) {
                            logger.error(">>>>>>>>>>>ishare-job, JobScheduleHelper#scheduleThread error:{}", e);
                        }
                    } finally {
                ...
                logger.info(">>>>>>>>>>>ishare-job, JobScheduleHelper#scheduleThread stop");
            }
        });
            ...
            public void run() {
                ...
                while (!ringThreadToStop) {
                    ...
                    try {
                        ...
                        // ring trigger
                        logger.debug(">>>>>>>>>>>ishare-job, time-ring beat : " + nowSecond + " = " + Arrays.asList(ringItemData) );
                        ...
                        if (!ringThreadToStop) {
                            logger.error(">>>>>>>>>>>ishare-job, JobScheduleHelper#ringThread error:{}", e);
                        }
                    }
                ...
                logger.info(">>>>>>>>>>>ishare-job, JobScheduleHelper#ringThread stop");
            }
        });
    ...
    private void pushTimeRing(int ringSecond, int jobId){
        ...
        logger.debug(">>>>>>>>>>>ishare-job, schedule push time-ring : " + ringSecond + " = " + Arrays.asList(ringItemData) );
    }
    ...
    public void toStop(){
        ...
        logger.info(">>>>>>>>>>>ishare-job, JobScheduleHelper stop");
    }
