...
import top.dddpeter.ishare.job.client.model.RegistryParamVO;
import top.dddpeter.ishare.job.client.util.BasicHttpUtil;
...
public class RegistryBaseClient {
    ...
    public RegistryBaseClient(String adminAddress, String accessToken, String biz, String env) {
        ...
        if (adminAddress==null || adminAddress.trim().length()==0) {
            throw new RuntimeException("ishare-registry adminAddress empty");
        }
        if (biz==null || biz.trim().length()<4 || biz.trim().length()>255) {
            throw new RuntimeException("ishare-registry biz empty Invalid[4~255]");
        }
        if (env==null || env.trim().length()<2 || env.trim().length()>255) {
            throw new RuntimeException("ishare-registry biz env Invalid[2~255]");
        }
    ...
    public Map<String, TreeSet<String>> discovery(Set<String> keys) {
        ...
        if (keys==null || keys.size()==0) {
            throw new RuntimeException("ishare-registry keys empty");
        }
